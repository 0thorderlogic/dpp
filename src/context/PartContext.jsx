...
export const PartProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (propellersData.length > 0) setSelectedPropeller(propellersData[0].Name);
  ...
  const othersTotalWeight = Object.values(otherWeights).reduce((a, b) => a + (parseFloat(b) || 0), 0);
  ...
  return (
    <PartContext.Provider value={{
      ...
      otherWeights, updateOtherWeight,
      propulsionWeight, totalWeight, othersTotalWeight,
      loading
    }}>
      {children}
